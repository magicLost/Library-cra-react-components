...
class ControlsFeatureController implements IControlsFeatureController {
  ...
  setState: React.Dispatch<
    ((prevState: CFState) => CFState) | CFState
  > | null = null;
  ...
  onItemMouseEnter = (event: any) => {
    ...
    this.setState((prevState: CFState) => {
      if (target.dataset && target.dataset.featureName) {
        ...
        if (prevState.title !== name) return { ...prevState, title: name };
      } else {
  ...
  private onItemPointerUp = (target: any) => {
    ...
    this.setState((prevState: CFState) => {
      if (prevState.isShowItems) {
        ...
        if (
          this.cfClasses.config.isMainItemText &&
          this.cfClasses.config.type === "TEXT"
        ) {
          mainItemText =
            index >= 0 ? this.items[index].title : prevState.mainItemText;
        }
